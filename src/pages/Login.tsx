...
const Login = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Dummy authentication - this would be replaced with a real auth system
    setTimeout(() => {
      setIsLoading(false);
      
      if (username.trim() && password.trim()) {
        // Simple validation just to make sure fields aren't empty
        toast.success("Login successful!");
        navigate('/dashboard');
      } else {
        toast.error("Please enter both username and password");
      }
    }, 800); // Short delay to simulate authentication process
  };
